...
static bool ensure_album_decode_worker(void) {
  ...
  if (xTaskCreatePinnedToCore(PhotoAlbumApp::decode_worker, "album_dec",
                              kDecodeStackBytes, nullptr, kDecodeTaskPriority,
                              &s_album_decode_worker, 1) != pdPASS) {
...
void PhotoAlbumApp::beginLoadTask(int idx, bool background) {
  ...
  if (!spawnDecodeTask(_session, idx)) {
    ...
    ESP_LOGE(TAG, "xTaskCreate failed (int=%u ext=%u B)",
